...
# Список секунд для отправки данных (для первого одноплатника: 00, 15, 30, 45)
# Для второго одноплатника: [5, 20, 35, 50]
# Для третьего одноплатника: [10, 25, 40, 55]
TARGET_SECONDS = [0, 15, 30, 45]
...
# Функция для отправки данных через UART с синхронизацией по времени
def send_data():
    last_sent_minute = -1
    last_sent_second = -1
    while running:
        try:
            ...
            # Проверяем, совпадает ли текущая секунда с одной из целевых
            if current_second in TARGET_SECONDS:
                # Проверяем, чтобы не отправлять дважды в ту же секунду
                if current_minute != last_sent_minute or current_second != last_sent_second:
                    ser.flushOutput()
                    message = f"S{drone_data['id']};{drone_data['x']};{drone_data['y']}E"
                    ser.write(message.encode('utf-8'))
                    print(f"Sent: {message} at {current_time.strftime('%Y-%m-%d %H:%M:%S.%f %Z')}")
                    last_sent_minute = current_minute
                    last_sent_second = current_second
            # Небольшая задержка для проверки времени, чтобы не перегружать процессор
            time.sleep(0.1)
        except serial.SerialException as e:
                    ...
                    if message.startswith('S') and message.endswith('E'):
                        try:
                            ...
                            if len(parts) >= 3:
                                ...
                                if len(parts) > 3:
                                    try:
                                        dBm = float(parts[3])  # Сохраняем dBm, если есть
                                ...
                                if drone_id != DRONE_ID:
                                    neighbors[drone_id] = {
                                        ...
                                        "x": x,
                                        "y": y,
                                        "timestamp": time.time(),  # всегда обновляем время
                                        "dBm": dBm
                                    }
